...
def suggest_player_names(request):
    try:
        ...
        # Include club names in suggestions
        clubs = set()
        for player in players:
            try:
                ...
                print(f"Error processing player {player.original_name}: {e}")
                continue

        club_data = {normalize_name(club): club for club in clubs}
        suggestions = find_close_matches(normalized_query, player_data)
        club_suggestions = find_close_matches(normalized_query, club_data)

        combined_suggestions = [player_data[match] for match in suggestions] + [club_data[match] for match in club_suggestions]
        return JsonResponse(combined_suggestions, safe=False)
    except Exception as e:
        print(f"Error in suggest_player_names view: {e}")
        return JsonResponse({'error': 'Internal server error'}, status=500)

def find_close_matches(name, data, cutoff=0.8):
    normalized_names = list(data.keys())
    matches = difflib.get_close_matches(name, normalized_names, n=5, cutoff=cutoff)
    return matches
def index(request):
